...
<div class="kop">
    ...
    <p>Laporan Rekap Lembur | {{ now()->format('d F Y') }}</p>
</div>
<h3>REKAP PENGAJUAN LEMBUR</h3>
<table>
    <thead>
        <tr>
            <th width="30">No</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{ \Carbon\Carbon::parse($p->tanggal_lembur)->format('d/m/Y') }}</td>
            <td>{{ $p->jam_mulai }}</td>
            <td>{{ $p->jam_selesai }}</td>
            <td>{{ $p->jumlah_jam }} jam</td>
...
<div class="footer">
    <p>Total: {{ $pengajuan->count() }} | Dicetak: {{ now()->format('d/m/Y H:i') }}</p>
</div>
</body>
</html>