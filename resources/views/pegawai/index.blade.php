...
<div class="flex justify-between items-center mb-6">
    <p class="text-gray-500">Daftar seluruh pegawai (Dosen & Tendik)</p>
    <div class="flex gap-2">
        <button onclick="document.getElementById('modal-import').classList.remove('hidden')"
            class="bg-green-600 hover:bg-green-700 px-4 py-2 rounded-lg text-white text-sm">
            📥 Import Excel
        </button>
        <a href="{{ route('pegawai.export') }}"
            class="bg-teal-600 hover:bg-teal-700 px-4 py-2 rounded-lg text-white text-sm">
            📤 Export Excel
        </a>
        <a href="{{ route('pegawai.create') }}"
            class="bg-blue-900 hover:bg-blue-800 px-4 py-2 rounded-lg font-medium text-white text-sm">
            + Tambah Pegawai
        </a>
    </div>
</div>
...
{{-- Modal Import --}}
<div id="modal-import" class="hidden z-50 fixed inset-0 flex justify-center items-center bg-black bg-opacity-50">
    <div class="bg-white shadow-xl p-6 rounded-xl w-full max-w-md">
        ...
        <form action="{{ route('pegawai.import') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="flex gap-3">
                <button type="submit" class="flex-1 bg-blue-900 hover:bg-blue-800 py-2 rounded-lg text-white text-sm">
                    Import Sekarang
                </button>
                <button type="button"
                    onclick="document.getElementById('modal-import').classList.add('hidden')"
                    class="flex-1 hover:bg-gray-50 py-2 border border-gray-300 rounded-lg text-sm">
                    Batal
                </button>
            </div>
        </form>
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50">
                ...
                <td class="space-x-2 px-6 py-4 text-center">
                    <a href="{{ route('pegawai.show', $pegawai->id) }}" class="text-green-600 hover:underline">Detail</a>
                    <a href="{{ route('pegawai.edit', $pegawai->id) }}" class="text-blue-600 hover:underline">Edit</a>
            ...
            @empty
            <tr>
                <td colspan="7" class="px-6 py-8 text-gray-400 text-center">Belum ada data pegawai.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
